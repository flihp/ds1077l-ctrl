#include "ds1077l.h"
#include "ds1077l-bus.h"
#include "ds1077l-div.h"
#include "ds1077l-mux.h"

#include <argp.h>
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>

#define DEV "/dev/i2c-1"
#define USAGE "usage: %s /dev/i2c-[0-9]\n"

static void
dump_args_dump (ds1077l_common_args_t *args)
{
    printf ("User provided options:\n");
    printf ("  address: 0x%x\n", args->address);
    printf ("  bus-dev: %s\n", args->bus_dev);
    printf ("  verbose: %s\n", args->verbose ? "true" : "false");
}

int main(int argc, char* argv[])
{
    ds1077l_bus_t bus = {0};
    ds1077l_div_t div = {0};
    ds1077l_mux_t mux = {0};
    int fd = 0;
    char* dev_path = NULL;
    ds1077l_common_args_t args = {0};

    if (argp_parse (&common_argp, argc, argv, 0, NULL, &args)) {
        perror ("argp_parse: \n");
        exit (1);
    }
    if (args.verbose == true)
        dump_args_dump (&args);
    fd = handle_get (args.bus_dev, args.address);
    if (fd == -1) {
        perror("handle_get: ");
        exit(1);
    }
    printf ("Current state of device 0x%x on bus %s:\n",
            args.address, args.bus_dev);
    if (bus_get(fd, &bus) == -1) {
        perror("bus_get: ");
        exit(1);
    }
    bus_pretty(&bus);
    if (mux_get(fd, &mux) == -1) {
        perror("mux_get: ");
        exit(1);
    }
    mux_pretty(&mux);
    if (div_get(fd, &div) == -1) {
        perror("div_get: ");
        exit(1);
    }
    div_pretty(&div);
    exit(0);
}
